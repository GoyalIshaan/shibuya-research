...
interface CrunchbaseConfig {
  enabled?: boolean;
  /**
   * Crunchbase API access is typically paid; this integration is optional.
   * Provide via env (CRUNCHBASE_API_KEY) or params.
   */
  apiKey?: string;
  /**
   * Base URL for the Crunchbase API you have access to.
   * Example (varies by plan/version): https://api.crunchbase.com
   */
  baseUrl?: string;
  /**
   * Fully-qualified endpoints to hit. This avoids hard-coding specific Crunchbase
...
export class CrunchbaseClient implements DataSource {
  name = 'crunchbase';

  async fetchSignals(params: CrunchbaseConfig): Promise<Signal[]> {
    const enabled = params?.enabled !== false;
    if (!enabled) return [];

    const apiKey = params.apiKey || process.env.CRUNCHBASE_API_KEY;
    const baseUrl = params.baseUrl || process.env.CRUNCHBASE_API_BASE_URL;
    const endpoints = params.endpoints || [];
    const timeoutMs = params.timeoutMs ?? 15000;

    if (!apiKey) {
      console.warn('[crunchbase] Missing CRUNCHBASE_API_KEY; skipping.');
      return [];
    }
    if (!baseUrl && endpoints.length === 0) {
      console.warn('[crunchbase] Missing CRUNCHBASE_API_BASE_URL and no endpoints provided; skipping.');
      return [];
    }
    ...
    const urls = endpoints.length
      ...
      : [{ name: 'root', url: baseUrl!, tags: ['crunchbase'] }];
      ...
      try {
        const resp = await axios.get(target.url, {
          ...
          headers: {
            ...
            // Keep auth generic; users can adjust if their plan requires query params instead.
        ...
        if (items && items.length > 0) {
          ...
          results.push({
            ...
            authorHandle: 'Crunchbase',
            timestamp: new Date(),
            url: target.url,
            text: `Crunchbase fetch: ${target.name}`.trim(),
            engagement: {},
            tags: ['crunchbase', ...target.tags],
            metadata: {
        ...
        console.error(`[crunchbase] Error fetching ${target.url}:`, err);
      }
    }
  ...
  private mapItemToSignal(
    ...
    const summary =
      ...
      '';
    const url =
      this.pickString(obj, mapping?.urlPath, ['url', 'homepage_url', 'website', 'cb_url']) ||
      target.url;
    const dateValue =
